...
class T5DenseReluDense(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.wi = MaskedLinear(config.d_model, config.d_ff, bias=False, pruning_method=config.pruning_method, mask_init=config.mask_init, mask_scale=config.mask_scale)
        self.wo = MaskedLinear(config.d_ff, config.d_model, bias=False, pruning_method=config.pruning_method, mask_init=config.mask_init, mask_scale=config.mask_scale)
        self.dropout = nn.Dropout(config.dropout_rate)

    def forward(self, hidden_states, threshold=None):
        h = self.wi(hidden_states, threshold=threshold)
        h = F.relu(h)
        h = self.dropout(h)
        h = self.wo(h, threshold=threshold)
        return h
...
class T5LayerFF(nn.Module):
    ...
    def forward(self, hidden_states, threshold=None):
        ...
        y = self.DenseReluDense(norm_x, threshold=threshold)
...
class T5Attention(nn.Module):
    def __init__(self, config: T5Config, has_relative_attention_bias=False, is_bidirectional=False):
        ...
        self.q = MaskedLinear(self.d_model, self.inner_dim, bias=False, pruning_method=config.pruning_method, mask_init=config.mask_init, mask_scale=config.mask_scale)
        self.k = MaskedLinear(self.d_model, self.inner_dim, bias=False, pruning_method=config.pruning_method, mask_init=config.mask_init, mask_scale=config.mask_scale)
        self.v = MaskedLinear(self.d_model, self.inner_dim, bias=False, pruning_method=config.pruning_method, mask_init=config.mask_init, mask_scale=config.mask_scale)
        self.o = MaskedLinear(self.inner_dim, self.d_model, bias=False, pruning_method=config.pruning_method, mask_init=config.mask_init, mask_scale=config.mask_scale)
    ...
    def forward(
        ...
        q = shape(self.q(input, threshold=threshold))  # (bs, n_heads, qlen, dim_per_head)

        if kv is None:
            k = shape(self.k(input, threshold=threshold))  # (bs, n_heads, qlen, dim_per_head)
            v = shape(self.v(input, threshold=threshold))  # (bs, n_heads, qlen, dim_per_head)
        elif past_key_value is None:
            k = v = kv
            k = shape(self.k(k, threshold=threshold))  # (bs, n_heads, qlen, dim_per_head)
            v = shape(self.v(v, threshold=threshold))  # (bs, n_heads, qlen, dim_per_head)
        ...
        context = self.o(context, threshold=threshold)
...
class T5Block(nn.Module):
    ...
    def forward(
        ...
        hidden_states = self.layer[-1](hidden_states, threshold=threshold)
